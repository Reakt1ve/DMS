from matrix_client.api import MatrixHttpApi
from contextlib import closing
import sys
import pymysql

def set_dbconnection ():
        connection = pymysql.connect(
                                        host='localhost',
                                        user='andrey',
                                        password='qwerty123',
                                        db='test',
                                        charset='utf8mb4'
                                    )
        return connection

def update_db():
        connection = set_dbconnection()
        with closing(connection):
                with connection.cursor() as cursor:
                        query = 'UPDATE users SET sended_notify = "1" WHERE room=\"' + '!ISscQXHtgMWNcYSEkQ:citis-matrix.ru' + '\"'
			cursor.execute(query)
			connection.commit()

matrix = MatrixHttpApi("https://citismatrix.zapto.org", token="")
response = matrix.send_message("!ISscQXHtgMWNcYSEkQ:citis-matrix.ru", sys.argv[1])
update_db()
